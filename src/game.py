...
class Game(object):
    ...
    def __init__(self, player_list, buying_price, selling_price, G, batinfo, pvinfo, prob):
        """TODO: to be defined.
    ...
    def solve(self):
        if self._model is None:
            ...
            self._selling_price, self._batinfo, self._pvinfo, self._prob)
            self._model = res[0]
            self._res = res
    ...
    def get_valfunc(self):
        if self._valfunc is None:
            ...
            L = range(self.N)
            for S in powerset(L):
                r = solve_centralized([pl[i] for i in S], pb, ps, binfo, pvinfo, pr)
