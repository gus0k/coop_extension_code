...
def solve_centralized(player_list, buying_price, selling_price):
    ...
    for n in set_N:
        for j in set_T:
            ...
            contributions[cons_bat_lb[(n, j)].name] = cont
    ...
    for n in set_N:
        for j in set_T:
            ...
            contributions[cons_bnd_ub[(n, j)].name] = cont
    ...
    for n in set_N:
        for j in set_T:
            cont = np.zeros(N)
            cont[n] = player_list[n]._ram
            contributions[cons_bnd_lb[(n, j)].name] = cont
    ...
    for j in set_T:
        cont = np.zeros(N)
        for n2 in set_N:
            cont[n2] = player_list[n2]._x[j]
        contributions[cons_z[j].name] = cont
    ...
    for j in set_T:
        ...
        for n2 in set_N:
            cont[n2] = - player_list[n2]._x[j]
        contributions[cons_zo[j].name] = cont
    ...
    return model, var, cons, df_cd, df_z, contributions
